...
| --- | --- | --- |
| GitHub Releases updater | Shortly after launch and every six hours while running (off in Settings), or when you choose Check for Updates | Version check against the configured `latest.json` endpoint; the signed bundle is downloaded only when you install |
